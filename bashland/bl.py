...
@click.option("-V",
             ...
             default=False,
             is_flag=True)
def bl(query, number, session, directory, system, interactive, duplicates, timestamps, version):
    """Bashhhub Search

    QUERY - Like string to search for
    """
    limit = number
    system_name = BL_SYSTEM_NAME if system else None
...
def run_interactive(commands):
    ...
    if command is not None:
        f = io.open(BL_HOME + '/response.bl', 'w+', encoding='utf-8')
