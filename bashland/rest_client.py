...
from .bashland_globals import BL_URL, BL_AUTH
...
user_agent = 'bashland/%s' % __version__
...
def json_auth_headers():
    return dict({'Authorization': 'Bearer {0}'.format(BL_AUTH())},
                **json_headers)
...
def get_command(uuid):
    url = BL_URL + "/api/v1/command/{0}".format(uuid)
    try:
...
def patch_system(system_patch, mac):
    ...
    url = BL_URL + "/api/v1/system/{0}".format(mac)
    ...
    try:
        ...
        if r.status_code in (403, 401):
            print("Permissons Issue. Run bashland setup to re-login.")
        return None
...
def search(limit=None, path=None, query=None, system_name=None, unique=None, session_id=None):
    ...
    url = BL_URL + "/api/v1/command/search"
...
def get_status_view(process_id, start_time):
    url = BL_URL + "/api/v1/client-view/status"
